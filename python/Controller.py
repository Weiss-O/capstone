    ...
    def moveAbsolute(self, theta_steps, phi_steps):
        ...
        if theta_relative != 0:
            if self.current_signs[0] != self.previous_signs[0]:
                ...
                theta_compensated = theta_relative + self.settings["backlash"]["theta"] * self.current_signs[0]
        if phi_relative != 0:
            if self.current_signs[1] != self.previous_signs[1]:
                #Add steps (depending on direction) to compensate for backlash
                phi_compensated = phi_relative + self.settings["backlash"]["phi"] * self.current_signs[1]
