...
def detect_person(img):
    ...
    img = torch.from_numpy(img.transpose(2, 0, 1)).float().unsqueeze(0) / 255.0 #(H,C,W) to (C,H,W) to tensor and normalize to [0,1]
...
if __name__ == "__main__":
    ...
    for i in range(2, 42):
        img = cv2.imread(f"test_set/capture_{i}.jpg")
